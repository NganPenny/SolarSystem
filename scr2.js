// Lưu trữ số lượt thích và không thích trong localStorage
function saveVote(planet, likeCount, dislikeCount) {
  localStorage.setItem(`${planet}-like`, likeCount);
  localStorage.setItem(`${planet}-dislike`, dislikeCount);
}

// Lưu trữ số tiền và lời nhắn từ quyên góp vào localStorage
function saveDonation(planet, amount, message) {
  localStorage.setItem(`${planet}-amount`, amount);
  localStorage.setItem(`${planet}-message`, message);
}

// Tăng số lượt thích hoặc không thích khi bấm vào nút
function vote(btn, planet, isLike) {
  var likeCount = parseInt(document.getElementById(`${planet}-like`).innerHTML);
  var dislikeCount = parseInt(document.getElementById(`${planet}-dislike`).innerHTML);

  if (isLike) {
    likeCount++;
    document.getElementById(`${planet}-like`).innerHTML = likeCount;
  } else {
    dislikeCount++;
    document.getElementById(`${planet}-dislike`).innerHTML = dislikeCount;
  }

  saveVote(planet, likeCount, dislikeCount);
}

// Quyên góp số tiền và lời nhắn
function donate(planet) {
  var messageInput = document.getElementById(`${planet}-message`);
  var message = messageInput.value;

  if (message.trim() !== "") {
    saveDonation(planet, 0, message);
    messageInput.value = "";
    showThankYou();
  }
}
...
// Khôi phục số lượt thích, không thích, số tiền và lời nhắn từ localStorage
function restoreData() {
  var planets = ['mercury', 'venus', 'earth', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune'];

  planets.forEach(function(planet) {
    var likeCount = localStorage.getItem(`${planet}-like`);
    var dislikeCount = localStorage.getItem(`${planet}-dislike`);
...
// Gọi hàm khôi phục số lượt thích, không thích, số tiền và lời nhắn sau khi tải trang
restoreData();
